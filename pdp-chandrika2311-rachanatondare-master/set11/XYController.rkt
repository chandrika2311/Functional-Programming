...
(begin-for-test
  (local
    ((define m (new Model%))
     (define cxy (new XYController% [model m])))
    (send cxy after-key-event "right")
    (send cxy model-after-button-down 310 210)
    (send cxy model-after-drag 300 270)
    (send cxy model-after-button-up 300 450)
    (send cxy details-image)
    (send cxy bgrnd-image (empty-scene 110 110))
    ))