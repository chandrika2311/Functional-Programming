...
(local
  ...
  (send pc1 after-key-event "left")
  (send pc1 details-image)
  (send pc1 after-button-down 250 250)
  (send pc1 after-key-event "left")
   (send pc1 after-key-event "left")
    (send pc1 after-key-event "left")
    (send pc1 after-key-event "up")
        (send pc1 after-key-event "down")
    (send pc1 after-key-event "down")
        (send pc1 model-after-button-down 300 250)
    (send pc1 model-after-drag 110 110)
    (send pc1 model-after-button-up 170 170 )
    (send pc1 details-image)
    (send pc1 bgrnd-image (empty-scene 150 150))
    ))